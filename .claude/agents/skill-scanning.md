...
| SK-IMG | Image Exfiltration | Markdown images with exfil query params (?data=, ?secret=, ?token=), raw IP image sources, variable interpolation (${}) or command substitution ($()) in image URLs | CRITICAL | LLM02 |
